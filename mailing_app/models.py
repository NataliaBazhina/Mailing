...
class Mailing(models.Model):
    ...
    frequency = models.CharField(max_length=2, choices=Frequency.choices, default=Frequency.DAILY, verbose_name='Периодичность')
    status = models.CharField(max_length=2, choices=Status.choices, default=Status.CREATED, verbose_name='Статус')
    clients = models.ManyToManyField(Client, verbose_name="Клиенты")
    mail = models.ForeignKey(Mail, on_delete=models.CASCADE, related_name='mails', verbose_name='Письмо')
    next_mailing = models.DateTimeField(null=True, blank=True, verbose_name='Следующая рассылка')
    is_immediate = models.BooleanField(default=False, verbose_name="Немедленная отправка")
    last_try = models.DateTimeField(null=True, blank=True, verbose_name='Последняя попытка')
...
class MailingTrying(models.Model):
    ...
    status_trying = models.CharField(max_length=30, choices=Status.choices, verbose_name="статус попытки")
